...
with the method of Laguerre.
   ...
   One odd thing has to be done here,  though.  There are maxima/minima
at +/- infinity,  too.  So if we find,  say,  three minima at x1, x2, x3,
...
there will be N+1 ranges to search.
...
of odd degree or the constant and leading terms are of opposite sign,  we
can bracket and remove a root quite readily,  then work on a polynomial
of lesser degree.  I've started on this.  I may not bother finishing it,
because the code is plenty fast enough for my needs at present;  the
slight loss of precision inherent in removing roots this way is more
worrisome than the slight gain in speed. */
...
but it's theoretically possible.  The use of this bisection code ensures
decent worst case behavior.
...
// printf( "Newt-Raph range %lf %lf (%lf %lf)\n", x1, x2, y1, y2);
   do
      {
      ...
      if( !y)
         ...
         use_bisection = 3; /* Laguerre has probably failed; bisect instead */
      else
         {
