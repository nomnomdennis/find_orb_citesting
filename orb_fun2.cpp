...
int link_arcs( OBSERVE *obs, int n_obs, const double r1, const double r2)
{
   ...
   fprintf( ofile, "Omega = %.5lf  incl = %.5lf\n",
   ...
   for( i = 0; i < n_obs; i++)
      ...
      fprintf( ofile, "Dist to target: %lf (%lf)\n", dist_to_target, obs[i].r);
...
//    r[i] = obs[i].solar_r;
   ...
   if( omega > PI + PI)
      omega -= PI + PI;
   q = 1. / (a * (1. + ecc));
   fprintf( ofile, "ecc = %lf  omega = %lf   q = %lf   a = %lf\n",
                  ...
                  double *orbit)
{
   ...
   *dt = angle * (*t0);
   *t0 *= 2. * PI;
// debug_printf( "Solar_r %lf, dt = %lf, t0 = %lf\n",
//          solar_r, *dt, *t0);
   if( orbit)
      {
               ...
               double *orbit, const int desired_soln)
{
   ...
   double r1 = 1.04, r2, radii[50];
   int n_solutions = 0, soln_type, types[50];

   debug_printf( "Delta_t = %lf\n", dt);
      ...
      if( t0_2)
         ...
         for( soln_type = 0; !bug_out; soln_type++)
            ...
            if( delta_1 * delta_2 < 0.)      /* we have a zero crossing */
               ...
               debug_printf( "Soln %d: type %d, r=%lf\n",
